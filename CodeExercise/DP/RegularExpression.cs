...
namespace CodeExercise.DP
{
    class RegularExpression
    {
        ...
        public bool IsMatch_recursiveTimeExceeded(string s, string p)
        {
            int M = s.Length;
            int N = p.Length;
            bool[,] F = new bool[M + 1, N + 1];
            ...
            bool ans = IsMatchHelper(s, p, F, visited, M, N);
            return ans;
        ...
        private bool IsMatchHelper(string s, string p, bool [,] F, bool[,] visited, int i, int j)
        {
            ...
            if (i == 0)
            {
                if (p[j-1] == '*')
                {
                    return IsMatchHelper(s, p, F, visited, i, j - 2);  // assume th patern is valid,   ""  "a*"
                }
                else
                {
                    return false;
            ...
            else if (p[j-1] == '*')
            {
                ...
                if (p[j-2] == s[i-1] || p[j-2] == '.')   // yic remember char need to match before going down
                {
